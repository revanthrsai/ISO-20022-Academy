...
Separating the envelope from the letter is the whole point: a bank's routing layer can read the BAH and decide where to send the message **without parsing the payment inside it**. Address on the outside, contents on the inside: the same reason the postal service works.
