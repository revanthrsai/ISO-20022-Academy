...
- **`EndToEndId` (`BOB-INV0042`)** is Bob's own reference. He set it in the pain.001; the pacs.008 carried it untouched; it surfaced again in Sweety's camt.054. It's why "Invoice 0042" was recognisable from the first tap to the closing statement.
- **`UETR`** is the globally-unique transaction reference stamped on the interbank leg. It's the answer to "where is my payment right now?", because every bank that touches the pacs.008 quotes the same `UETR`.
